...
Thus, we rewrite \eqref{eq:armijo} as
	...
	\par In particular \eqref{eq:induction1} and \eqref{eq:induction2} are also valid if we replace $r(k)$ with $R(k):=r(k+W+1)$ (by simply redefining $k$). Now, for $k$ sufficiently large, we have that
...
\begin{proposition}
	Given $\{x_k\}_k$ the sequence generate by Algorithm \eqref{eq:bb1} to solve \eqref{eq:quadratic}, with $Q$ symmetric and positive definite, with $\lambda_n<2\lambda_1$, we get 
	\begin{equation*}
...
Exercise.
\end{proof}
\noindent Beyond this result, the same type of convergence (with a constant $\delta<1$) can be proved also without assuming $\lambda_n<2\lambda_1$. %TODO type of convergence

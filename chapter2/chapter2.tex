...
\begin{theorem}[Convergence of GD with Armijo Line Search (part 2)]\label{thm:Armijo_convergence_p2}
	Let $f\in \C(\Rn)$ and let $\{x_k\}$ be a sequence generated by Algorithm \ref{alg:gd_armijo}. Let the level set $\mathcal{L}_0:=\{x\in \R^n: f(x)\leq f(x_0)\}$ be compact. Assume that $f$ is bounded from below over $\Rn$. Then we have $\grad(x_k) \to 0$ as $k\to \infty$.
\end{theorem}
\begin{proof}
	...
	\end{equation}
Therefore, as $\nabla f(x_k) \neq 0$
\begin{align}\label{inq:gradient_converges}
(1-c) \|\nabla f(x_k)\| \leq \|\nabla f(x_k) - \nabla f(z_k)\|,
\end{align}
For some $\gamma>0$, denote by $\mathcal{L}_0^\gamma$ the closed $\gamma$ neighborhood around $\mathcal{L}_0$, i.e. $L_0^\gamma = \{x\in \R^n : \text{dist}\{x, \mathcal{L}_0\}\leq \gamma\}$. 
Because of \eqref{eq:hatt_limit}, we have $\|z_k - x_k\|\to 0$, hence for a $\gamma>0$ there exists a $\bar{k}>0$ such that for all $k >\bar{k}, k\in K$ we have $\|z_k-x_k\|\leq \gamma$, which implies $x_k, z_k \in \mathcal{L}_0^\gamma $. 
As $\nabla f$ is continuous, it is uniformly continuous on the compact set, hence 
\begin{equation*}
\lim_{i \to \infty}        \|\nabla f(z_k) - \nabla f(x_k)\| = 0,
\end{equation*}
which together with \eqref{inq:gradient_converges} brings to a contradiction.
\end{proof}
...
\end{lemma}
\begin{proof}
	If we look for the stationary point of the problem above, we get that $\nabla g(x) = \grad(x_k) + \frac{1}{t_k}(x-x_k)=0$, thus $x = x_k - t_k \grad(x_k)$. Since the Hessian of $g(x)$ is positive definite for any $x \in \Rn$ (i.e., $\nabla^2 g(x) = \frac{1}{t_k}I $), by Theorem 1.12 (the sufficient condition for a global minimizer) we have that each stationary point is a global minimizer. 
\end{proof}
...
\end{theorem}
\begin{proof}
	Suppose that $\hess(x) \succcurlyeq 0$ for all $x \in \Rn$. We will prove \eqref{eq:grad_ineq} which is enough to establish convexity. Let $x,y\in \Rn$, then by the Mean Value Theorem$^2$ (Theorem 1.6 from Chapter 1) we get that there exists $z\in[x,y]$ for which 
	...
	\end{equation*}
	In addition, by the quadratic approximation theorem (Theorem 1.4 from Chapter 1), we have that 
	\begin{equation*}
...
\begin{proof}
	...
	\end{align*}
This property is called Fej\'er monotonicity w.r.t. the distance from $x^*$. Thus, $\|x_k - x^*\|^2$ is a non-increasing sequence, from which we get 
...
\begin{algorithm}[H]\label{alg:bb}
	\caption{Nonmonotone Barzilai-Borwein Method (\cite{raydan97a})}
...
\begin{remark} $\;$\\
	\vspace{-5mm}
	\begin{itemize}
		\item The same finite convergence can be obtained also when $Q$ is positive semi-definite, by assuming that $f$ admits a minimum (proof left as an Exercise). %it is proved in Proposition 12.10 page 272 of the Grippo Sciandrone
		...
		Indeed, up to an additive constant, the problem above is a quadratic function with $Q=A^TA$ and $c=A^Tb$, that always admits a solution in $\Rn$ i.e., $A^TA x =A^Tb$ (proof left as an Exercise). %pp 43-44 Proposition 2.11 from Grippo Sciandrone 
		\item The expression for $\beta_{k+1}$ can be simplified in a way that the Hessian matrix does not appear explicitly, i.e., $\beta_{k+1} =\frac{||\grad(x_{k+1})||^2}{||\grad(x_k)||^2}$ (Exercise). %pp 255-256 to prove (12.25) from Grippo Sciandrone 

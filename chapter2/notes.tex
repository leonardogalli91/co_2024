...
\begin{proof}
	...
	\begin{equation}\label{eq:tower}
		f(x_0) - f(x_{T+1}) = \frac{1}{L}\sum_{k=0}^{T} \|\grad(x_k)\|^2
	\end{equation}
...
\end{equation*}
\noindent Moreover, the asymptotic convergence of Algorithm \ref{alg:gd_armijo} can also be proven if we assume $f\in \C(\Rn)$ instead of $f\in \LC(\Rn)$.

\begin{theorem}[Convergence of GD with Armijo Line Search]\label{thm:Armijo_convergence}
	Let $f\in \C(\Rn)$ and let $\{x_k\}_k$ be a sequence generated by Algorithm \ref{alg:armijo}. Assume that $f$ is bounded below over $\Rn$. Then we have the following
	\begin{itemize}

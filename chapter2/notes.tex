...
\end{equation*}
Notice that $\alpha_k = \frac{d_k^T Q(x^* - x_0)}{d_k^T Q d_k} = \frac{d_k^T Q(x^* - x_k)}{d_k^T Q d_k} + \frac{d_k^T Q(x_k - x_0)}{d_k^T Q d_k}$. From $x_k - x_0 = \sum_{i=0}^{k-1} \alpha_i d_i$, and the $Q$-conjugacy assumption, we have
\begin{equation*}
	d_k^T Q(x_k - x_0) = 0. 
\end{equation*}
...
\end{equation}
where now the step size $\alpha_k$ does not involve $v$ nor $x^*$.


\begin{proposition}[Finite Convergence for Conjugate Direction Method]
...
\end{proposition}
\begin{proof}
	Let $k \geq 1$ and suppose that $\grad(x_i) \neq 0$, for $i = 0, 1, \ldots, k - 1$. Starting from a point $x_i$ of the sequence, and pre-multiplying by $Q$, we can write:
	\begin{equation*}
		Q x_k = Q x_i + \sum_{j=i}^{k-1} \alpha_j Q d_j,
	\end{equation*}
and, given $\grad(x) = Qx - c$, we also have
\begin{equation*}
...
\end{equation}
For this method we have the following properties.
\begin{lemma}\label{lemma:cg}
	Let $f(x) =\frac{1}{2}x^TQx -c^Tx$, with $Q\in \Rnn$ symmetric and positive definite, then for the Algorithm \eqref{eq:cg}, we have 
	\begin{enumerate}
...
\end{lemma}
\begin{proof}
	$(i)$: $d_{k+1}^TQd_k = -\grad(x_{k+1}) + \grad(x_{k+1}) = 0$. \\
	$(ii)$: Pre-multiplying by $Q$ both members of the update rule, by $\grad(x) = Qx - c$ we obtain
	\begin{equation}\label{eq:12.11}
		\grad(x_{k+1}) = \grad(x_k) + \alpha_k Q d_k. 
	\end{equation}
Now, we pre-multiply by $d_k^T$ and apply \eqref{eq:conjugate_alpha}, to get 
$$d_k^T\grad(x_{k+1}) = d_k^T\grad(x_k) + \alpha_k d_k^TQ d_k = d_k^T\grad(x_k) -\frac{\grad (x_k)^T d_k}{d_k^T Q d_k} d_k^TQ d_k.$$
$(iii)$: Pre-multiplying \eqref{eq:cg} by $\grad(x_{k+1})^T$ we get
$$\grad(x_{k+1})^Td_{k+1} = -\grad(x_{k+1})^T\grad(x_{k+1}) +\beta_{k+1} \grad(x_{k+1})^Td_k,$$
which brings to $(iii)$, because the last term is 0, because of $(ii)$.\\
...
\begin{proposition}[CGM: Conjugacy and Convergence]
	Let $f$ be the quadratic function defined by $f(x) = \frac{1}{2}x^T Q x - c^T x$, with $Q$ symmetric and positive definite. Suppose $g_0 \neq 0$ and consider the points $x_k$, for $k = 1, 2, \ldots$ generated by the CGM defined by (12.5)--(12.9). Then there exists an integer $m \leq n - 1$ such that, for $i = 1, \ldots, m$ we have
	\begin{align}
		g_i^T g_j &= 0, \quad j = 0, 1, \ldots, i - 1, \tag{12.15} \\
		d_i^T Q d_j &= 0 \quad j = 0, 1, \ldots, i - 1, \tag{12.16} \\
		g_{m+1} &= 0. \tag{12.17}
	\end{align}
\end{proposition}

\begin{proof}
	As $g_0 \neq 0$ we can assume that there exists an integer $m > 0$ such that $g_i \neq 0$ for $i = 0, 1 \ldots, m$ and we show, by induction, that (12.15) and (12.16) hold.
...
\begin{align}
	g_{i+1}^T g_j &= 0, \quad j = 0, 1, \ldots, i, \tag{12.18} \\

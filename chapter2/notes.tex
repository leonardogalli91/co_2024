...
\begin{lemma}[Co-coercivity]
	...
	\begin{align}
		...
		\frac{1}{L} ||\grad(y)-\grad(x)||^2 \leq (\grad(x)-\grad(y))^T(y-x)\label{eq:cocoercivity}
	\end{align}
\end{lemma}
\begin{proof}
...
\begin{equation*}
	\begin{split}
		f(x) - f(y) &\leq \grad(x)^T(x-z) -\frac{1}{L}||\grad(y)-\grad(x)||^2 + \frac{1}{2L}||\grad(y)-\grad(x)||^2\\
		&= \grad(x)^T(x-z)-\frac{1}{2L}||\grad(y)-\grad(x)||^2,
	\end{split}
\end{equation*}
...
\end{proof}
For simplicity, we will assume to know $L$, so that we can employ Algorithm \eqref{gd}, i.e. $t=\frac{1}{L}$
...
\end{theorem}
\begin{proof}
	Let $f$ be convex and $L$-smooth. From \eqref{eq:cocoercivity} applied on $x_k$ and $x^*$, and remembering that $x^*$ is a stationary point, it follows that
	\begin{align*}
...
Notice that by \eqref{eq:pre_cocoercivity} applied on $y=x$ and $x=x^*$, we have $ \frac{1}{2L}\|\grad(x_k) \|^2 \leq f(x)-f^*$, meaning that the actual termination criterion of Algorithm \ref{gd} on the norm of the gradient also works for convex functions. In particular, we have the following.
\begin{corollary}
